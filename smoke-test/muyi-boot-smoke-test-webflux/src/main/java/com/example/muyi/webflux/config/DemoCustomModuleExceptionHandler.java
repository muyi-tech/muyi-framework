 ...
 * 同一实现注册两条通道：
 * <ul>
 * <li>通道一（@ExceptionHandler）：WebFlux 全局异常处理直接分发，不依赖核心
 * 处理器接线，按 advice 顺序（HIGHEST_PRECEDENCE）抢在核心兜底前翻译专有异常；</li>
 * <li>通道二（SPI 扩展点）：实现 {@link ReactiveModuleExceptionHandler}，由核心
 * 处理器收集分发，返回 {@code null} 放行交回核心兜底。</li>
 * </ul>
 *
 * @author keep simple
 * @since 2026/9/14
...
public class DemoCustomModuleExceptionHandler implements ReactiveModuleExceptionHandler {

    /**
     * 通道一：WebFlux 全局异常处理（按异常类型匹配，不匹配不进入本方法）。
     * @param ex 业务专有异常
     * @return 错误结果
     */
